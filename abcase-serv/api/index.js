...
const options = {
  ...
  port: 443,
  path: '/b/5e99c0b05fa47104cea263c6/latest',
  method: 'GET',
  headers: {
    'secret-key': '$2b$10$rQH.eiLiF0GGvFRVSehDNeH3eJCDZ7yPOAw6eWR8pM6hLMvbRCShi',
  }
}
...
function get_json_data(){
  const jsonapireq = https.request(options, res => {
    ...
    res.on('data', d => {
      ...
      var buffer = Buffer.concat(dd);
      reports = JSON.parse(buffer.toString('utf8'));
      generate();
    })
  })
  jsonapireq.on('error', error => {
    console.error(error)
  })
  jsonapireq.end()
}
...
function generate(){
  ...
  for (const [i, aReport] of reports.entries()) {
      ...
      aReport['summary'] = summary_new_cases + 
                      summary_new_deaths + 
